...
public class EmbarcacionController {
    ...
    public ResponseEntity<Embarcacion> updateEmbarcacion(@RequestBody Embarcacion embarcacion, @PathVariable Integer id){
        Embarcacion embarcacionActualizada = embarcacionService.updateEmbarcacion(embarcacion, id);
        if (embarcacionActualizada != null){
            return ResponseEntity.ok(embarcacionActualizada);
        }
        return ResponseEntity.notFound().build();
    }
